...
`)])}function Ts(){return Tr()}function Tr(){let e=Q(32),t=rt.encodeFromBytes("AGE-SECRET-KEY-",e).toUpperCase();return Promise.resolve(t)}function Is(){let e=Q(32),t=rt.encodeFromBytes("AGE-SECRET-KEY-PQ-",e).toUpperCase();return Promise.resolve(t)}async function Us(e){let t;if(Rs(e))t=e;else if(e.startsWith("AGE-SECRET-KEY-PQ-1")){let r=rt.decodeToBytes(e);if(r.prefix.toUpperCase()!=="AGE-SECRET-KEY-PQ-"||r.bytes.length!==32)throw Error("invalid identity");let o=rn.getPublicKey(r.bytes);return rt.encode("age1pq",rt.toWords(o),!1)}else{let r=rt.decodeToBytes(e);if(!e.startsWith("AGE-SECRET-KEY-1")||r.prefix.toUpperCase()!=="AGE-SECRET-KEY-"||r.bytes.length!==32)throw Error("invalid identity");t=r.bytes}let n=await ke(t);return rt.encodeFromBytes("age",n)}var an=class{recipient;constructor(t){let n=rt.decodeToBytes(t);if(!t.startsWith("age1pq1")||n.prefix.toLowerCase()!=="age1pq"||n.bytes.length!==1216)throw Error("invalid recipient");this.recipient=n.bytes}wrapFileKey(t){let{cipherText:n,sharedSecret:r}=rn.encapsulate(this.recipient),o=new TextEncoder().encode("age-encryption.org/mlkem768x25519"),{key:s,nonce:i}=gn(ks,r,o),c=ht(s,i).encrypt(t);return[new dt(["mlkem768x25519",tt.encode(n)],c)]}},ln=class{identity;constructor(t){let n=rt.decodeToBytes(t);if(!t.startsWith("AGE-SECRET-KEY-PQ-1")||n.prefix.toUpperCase()!=="AGE-SECRET-KEY-PQ-"||n.bytes.length!==32)throw Error("invalid identity");this.identity=n.bytes}unwrapFileKey(t){for(let n of t){if(n.args.length<1||n.args[0]!=="mlkem768x25519")continue;if(n.args.length!==2)throw Error("invalid mlkem768x25519 stanza");let r=tt.decode(n.args[1]);if(r.length!==1120||n.body.length!==32)throw Error("invalid mlkem768x25519 stanza");let o=rn.decapsulate(r,this.identity),s=new TextEncoder().encode("age-encryption.org/mlkem768x25519"),{key:i,nonce:c}=gn(ks,o,s);try{return ht(i,c).decrypt(n.body)}catch{continue}}return null}},ks=25722,kc=80,Lr=16;function gn(e,t,n){let r=Rc(e),o=fn(r,void 0,"psk_id_hash",new Uint8Array(0)),s=fn(r,void 0,"info_hash",n),i=new Uint8Array(1+o.length+s.length);i[0]=0,i.set(o,1),i.set(s,1+o.length);let c=fn(r,t,"secret",new Uint8Array(0)),f=_r(r,c,"key",i,32),l=_r(r,c,"base_nonce",i,12);return{key:f,nonce:l}}function Rc(e){let t=new Uint8Array(10);return t.set(new TextEncoder().encode("HPKE"),0),t[4]=e>>8&255,t[5]=e&255,t[6]=0,t[7]=1,t[8]=0,t[9]=3,t}function fn(e,t,n,r){let o=new Uint8Array(7+e.length+n.length+r.length),s=0;return o.set(new TextEncoder().encode("HPKE-v1"),s),s+=7,o.set(e,s),s+=e.length,o.set(new TextEncoder().encode(n),s),s+=n.length,o.set(r,s),Bt($,o,t)}function _r(e,t,n,r,o){let s=new Uint8Array(9+e.length+n.length+r.length),i=0;return s[i]=o>>8&255,s[i+1]=o&255,i+=2,s.set(new TextEncoder().encode("HPKE-v1"),i),i+=7,s.set(e,i),i+=e.length,s.set(new TextEncoder().encode(n),i),i+=n.length,s.set(r,i),de($,t,s,o)}function Oc(e){e.length!==_t.lengths.publicKeyUncompressed&&(e=_t.Point.fromBytes(e).toBytes(!1));let t=_t.utils.randomSecretKey(),n=_t.getPublicKey(t,!1),r=_t.getSharedSecret(t,e,!0).subarray(1),o=new Uint8Array(n.length+e.length);o.set(n,0),o.set(e,n.length);let s=new Uint8Array(5);s.set(new TextEncoder().encode("KEM"),0),s[3]=Lr>>8,s[4]=Lr&255;let i=fn(s,void 0,"eae_prk",r),c=_r(s,i,"shared_secret",o,32);return{encapsulatedKey:n,sharedSecret:c}}var un=class{recipient;constructor(t){let n=rt.decodeToBytes(t);if(!t.startsWith("age1tag1")||n.prefix.toLowerCase()!=="age1tag"||n.bytes.length!==33)throw Error("invalid recipient");this.recipient=n.bytes}wrapFileKey(t){let{encapsulatedKey:n,sharedSecret:r}=Oc(this.recipient),o=new TextEncoder().encode("age-encryption.org/p256tag"),s=(()=>{let l=$(this.recipient).subarray(0,4),a=new Uint8Array(n.length+l.length);return a.set(n,0),a.set(l,n.length),Bt($,a,o).subarray(0,4)})(),{key:i,nonce:c}=gn(Lr,r,o),f=ht(i,c).encrypt(t);return[new dt(["p256tag",tt.encode(s),tt.encode(n)],f)]}},dn=class{recipient;constructor(t){let n=rt.decodeToBytes(t);if(!t.startsWith("age1tagpq1")||n.prefix.toLowerCase()!=="age1tagpq"||n.bytes.length!==1249)throw Error("invalid recipient");this.recipient=n.bytes}wrapFileKey(t){let{cipherText:n,sharedSecret:r}=Es.encapsulate(this.recipient),o=new TextEncoder().encode("age-encryption.org/mlkem768p256tag"),s=(()=>{let l=$(this.recipient.subarray(1184)).subarray(0,4),a=new Uint8Array(n.length+l.length);return a.set(n,0),a.set(l,n.length),Bt($,a,o).subarray(0,4)})(),{key:i,nonce:c}=gn(kc,r,o),f=ht(i,c).encrypt(t);return[new dt(["mlkem768p256tag",tt.encode(s),tt.encode(n)],f)]}},hn=class{recipient;constructor(t){let n=rt.decodeToBytes(t);if(!t.startsWith("age1")||n.prefix.toLowerCase()!=="age"||n.bytes.length!==32)throw Error("invalid recipient");this.recipient=n.bytes}async wrapFileKey(t){let n=Q(32),r=await ke(n),o=await on(n,this.recipient),s=new Uint8Array(r.length+this.recipient.length);s.set(r),s.set(this.recipient,r.length);let i=new TextEncoder().encode("age-encryption.org/v1/X25519"),c=kt($,o,s,i,32);return[new dt(["X25519",tt.encode(r)],xn(t,c))]}},Re=class{identity;recipient;constructor(t){if(Rs(t)){this.identity=t,this.recipient=ke(t);return}let n=rt.decodeToBytes(t);if(!t.startsWith("AGE-SECRET-KEY-1")||n.prefix.toUpperCase()!=="AGE-SECRET-KEY-"||n.bytes.length!==32)throw Error("invalid identity");this.identity=n.bytes,this.recipient=ke(n.bytes)}async unwrapFileKey(t){for(let n of t){if(n.args.length<1||n.args[0]!=="X25519")continue;if(n.args.length!==2)throw Error("invalid X25519 stanza");let r=tt.decode(n.args[1]);if(r.length!==32)throw Error("invalid X25519 stanza");let o=await on(this.identity,r),s=await this.recipient,i=new Uint8Array(r.length+s.length);i.set(r),i.set(s,r.length);let c=new TextEncoder().encode("age-encryption.org/v1/X25519"),f=kt($,o,i,c,32),l=bn(n.body,f);if(l!==null)return l}return null}},pn=class{passphrase;logN;constructor(t,n){this.passphrase=t,this.logN=n}wrapFileKey(t){let n=Q(16),r="age-encryption.org/v1/scrypt",o=new Uint8Array(r.length+16);o.set(new TextEncoder().encode(r)),o.set(n,r.length);let s=Zn(this.passphrase,o,{N:2**this.logN,r:8,p:1,dkLen:32});return[new dt(["scrypt",tt.encode(n),this.logN.toString()],xn(t,s))]}},yn=class{passphrase;constructor(t){this.passphrase=t}unwrapFileKey(t){for(let n of t){if(n.args.length<1||n.args[0]!=="scrypt")continue;if(t.length!==1)throw Error("scrypt recipient is not the only one in the header");if(n.args.length!==3||!/^[1-9][0-9]*$/.test(n.args[2]))throw Error("invalid scrypt stanza");let r=tt.decode(n.args[1]);if(r.length!==16)throw Error("invalid scrypt stanza");let o=Number(n.args[2]);if(o>20)throw Error("scrypt work factor is too high");let s="age-encryption.org/v1/scrypt",i=new Uint8Array(s.length+16);i.set(new TextEncoder().encode(s)),i.set(r,s.length);let c=Zn(this.passphrase,i,{N:2**o,r:8,p:1,dkLen:32}),f=bn(n.body,c);if(f!==null)return f}return null}};function xn(e,t){let n=new Uint8Array(12);return ht(t,n).encrypt(e)}function bn(e,t){if(e.length!==32)throw Error("invalid stanza");let n=new Uint8Array(12);try{return ht(t,n).decrypt(e)}catch{return null}}function Rs(e){return typeof CryptoKey<"u"&&e instanceof CryptoKey}var Qt=16,Ur=64*1024,Ir=Ur+Qt;function Os(e){let t=new Uint8Array(12),n=()=>{for(let i=t.length-2;i>=0&&(t[i]++,t[i]===0);i--);},r=!0,o=new Uint8Array(Ir),s=0;return new TransformStream({transform(i,c){for(;i.length>0;){if(s===o.length){let l=ht(e,t).decrypt(o);c.enqueue(l),n(),s=0,r=!1}let f=Math.min(o.length-s,i.length);o.set(i.subarray(0,f),s),s+=f,i=i.subarray(f)}},flush(i){t[11]=1;let c=ht(e,t).decrypt(o.subarray(0,s));if(!r&&c.length===0)throw new Error("final chunk is empty");i.enqueue(c)}})}function Ks(e){if(e<Qt)throw Error("ciphertext is too small");if(e===Qt)return 0;let t=Math.floor(e/Ir),n=e%Ir;if(0<n&&n<=Qt)throw Error("ciphertext size is invalid");let r=e;return r-=t*Qt,r-=n>0?Qt:0,r}function Cs(e){let t=new Uint8Array(12),n=()=>{for(let s=t.length-2;s>=0&&(t[s]++,t[s]===0);s--);},r=new Uint8Array(Ur),o=0;return new TransformStream({transform(s,i){for(;s.length>0;){if(o===r.length){let f=ht(e,t).encrypt(r);i.enqueue(f),n(),o=0}let c=Math.min(r.length-o,s.length);r.set(s.subarray(0,c),o),o+=c,s=s.subarray(c)}},flush(s){t[11]=1;let i=ht(e,t).encrypt(r.subarray(0,o));s.enqueue(i)}})}function Hs(e){let t=Math.max(1,Math.ceil(e/Ur));return e+Qt*t}var kr={};An(kr,{decode:()=>Cc,encode:()=>Kc});function Kc(e){let t=[];t.push(`-----BEGIN AGE ENCRYPTED FILE-----
